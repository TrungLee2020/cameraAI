...
static void msg_func_rface(gsf_msg_t *req, int isize, gsf_msg_t *rsp, int *osize)
{
    if(req->set)
    {
        gsf_svp_rface_t *rfacecfg = (gsf_svp_rface_t*)req->data;
        
        // Lưu cấu hình RetinaFace
        svp_parm.rface = *rfacecfg;
        json_parm_save(svp_parm_path, &svp_parm);
        ...
        if(1) {
            GSF_MSG_DEF(gsf_polygons_t, lines, 8*1024);
            lines->polygon_num = 4;
            lines->polygons[3].point_num = rfacecfg->det_polygon.polygons[0].point_num;
            memcpy(lines->polygons[3].points, 
                   rfacecfg->det_polygon.polygons[0].points,
                   sizeof(lines->polygons[3].points));
            GSF_MSG_SENDTO(GSF_ID_APP_LINES, 0, SET, 0, 
                          sizeof(gsf_polygons_t), GSF_IPC_APP, 2000);
        }
    }
    else
    {
        gsf_svp_rface_t *rfacecfg = (gsf_svp_rface_t*)rsp->data;
        
        // Lấy cấu hình hiện tại
        memcpy(rfacecfg, &svp_parm.rface, sizeof(svp_parm.rface));
        
        rsp->err  = 0;
        rsp->size = sizeof(gsf_svp_rface_t);
    }
}
...
static msg_func_t *msg_func[GSF_ID_SVP_END] = {
    ...
    [GSF_ID_SVP_RFACE]  = msg_func_rface,
 };
