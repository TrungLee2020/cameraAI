...
static void _handle_sig(int signo)
{
    if (signo == SIGINT || signo == SIGTERM) 
    {
       ...
       extern int face_stop(); face_stop();
    }
    exit(-1);
}
...
int main(int argc, char *argv[])
{
    ...
    extern int face_start(char *home_path);
    extern int face_stop();
    ...
    {                        
      ...
      if(face_alg != svp_parm.svp.face_alg)
      {
        if(face_alg == 0 && svp_parm.svp.face_alg > 0)
        {
          face_start(home_path);
        }
        else if(face_alg > 0 && svp_parm.svp.face_alg == 0)
        {
          face_stop();
        }
        face_alg = svp_parm.svp.face_alg;  
      }
      sleep(1);
    }
